...
xpose4()
getwd()
table <- read.table('CTL/sdtab2_14.txt', skip = 1, header = T)
head(table)
table %>%
  filter(MDV == 0) %>%
...
table %>%
  filter(MDV == 0, BQL != 1, ROUTE == 2) %>%
  ggplot() +
  geom_point(aes(x = TIME, y = DV), color = "black") +
  geom_point(aes(x = TIME, y = IPRED), color = "red") +
  geom_line(aes(x = TIME, y = IPRED), color = "red") +
  facet_wrap(.~as.factor(ID), scale = "free_y") +
